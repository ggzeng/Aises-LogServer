...
class TestConfigAPI:
    ...
    def test_update_config_valid(self, client):
        ...
        response = client.put(
            "/api/config",
            json={"max_logs_per_client": new_value}
        )
    ...
    def test_update_config_invalid_too_large(self, client):
        ...
        response = client.put(
            "/api/config",
            json={"max_logs_per_client": 200000}
        )
    ...
    def test_update_config_invalid_type(self, client):
        ...
        response = client.put(
            "/api/config",
            json={"max_logs_per_client": "invalid"}
        )
    ...
    def test_update_config_boundary_values(self, client):
        ...
        response = client.put(
            "/api/config",
            json={"max_logs_per_client": 100000}
        )
        assert response.status_code == 200
